...
export async function getPlaceImage(placeName, city) {
  ...
  if (GOOGLE_KEY) {
    try {
      const searchUrl = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${encodeURIComponent(`${placeName} ${city}`)}&inputtype=textquery&fields=photos,place_id&key=${GOOGLE_KEY}`;
      ...
      if (searchData?.candidates?.length > 0 && searchData.candidates[0].photos?.length > 0) {
        ...
        const imageUrl = `https://maps.googleapis.com/maps/api/place/photo?maxwidth=800&photoreference=${photoRef}&key=${GOOGLE_KEY}`;
  ...
  if (UNSPLASH_KEY) {
    const query = `"${placeName}" ${city} exterior landmark`;
    try {
  ...
  const genericFallback = "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?q=80&w=600";
