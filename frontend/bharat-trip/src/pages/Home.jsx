...
const TravelFeed = () => {
  ...
  const trending = [
    { id: 5001, title: "Golden Triangle Luxury", city: "Delhi, Jaipur", days: 5, cost: 42000, tags: ["History", "Royal"], img: "https://images.unsplash.com/photo-1599661046289-e31897846e41?q=80&w=800" },
    { id: 5002, title: "Kerala Backwater Bliss", city: "Alleppey", days: 4, cost: 22000, tags: ["Nature", "Relaxed"], img: "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?q=80&w=800" },
    { id: 5003, title: "Goa Beach Party", city: "North Goa", days: 3, cost: 15000, tags: ["Party", "Beach"], img: "https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?q=80&w=800" }
  ];

  const ideas = [
    { id: 5004, title: "Spirituality in Kashi", city: "Varanasi", days: 3, cost: 12000, tags: ["Culture", "Spiritual"], img: "https://images.unsplash.com/photo-1561361513-2d000a50f0dc?q=80&w=800" },
    { id: 5005, title: "Yoga Retreat", city: "Rishikesh", days: 5, cost: 18000, tags: ["Wellness", "Adventure"], img: "https://images.unsplash.com/photo-1545105511-923f63f29e07?q=80&w=800" }
  ];
  ...
  const FeedRow = ({ title, items, badge }) => (
    <div style={{ marginBottom: '60px', display: 'block', width: '100%' }}>
      <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h2 style={{ fontSize: '1.8rem', fontWeight: '800', color: '#ffffff', margin: 0 }}>{title}</h2>
        {badge && <span style={{ background: '#3b82f6', color: '#ffffff', padding: '4px 14px', borderRadius: '100px', fontSize: '0.75rem', fontWeight: '800' }}>{badge}</span>}
      </div>
        ...
        {items.map(item => (
          ...
          >
            <div style={{ width: '100%', height: '180px', background: '#000', position: 'relative', display: 'block' }}>
              ...
              <span style={{ position: 'absolute', top: '16px', left: '16px', background: '#3b82f6', color: '#ffffff', padding: '5px 12px', borderRadius: '8px', fontSize: '0.7rem', fontWeight: '900', zIndex: 10 }}>{badge || 'HOT'}</span>
            </div>
            <div style={{ padding: '24px', background: '#1a1a1f', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', color: '#94a3b8', fontWeight: '700' }}>
                <span>📍 {item.city}</span>
                <span>{item.days}d</span>
              </div>
              <h3 style={{ fontSize: '1.25rem', fontWeight: '800', color: '#ffffff', margin: '0', display: 'block' }}>{item.title}</h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {item.tags.map(t => <span key={t} style={{ background: 'rgba(59, 130, 246, 0.2)', color: '#60a5fa', padding: '4px 10px', borderRadius: '8px', fontSize: '10px', fontWeight: '800' }}>{t}</span>)}
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255, 255, 255, 0.1)', paddingTop: '16px', marginTop: '4px' }}>
                <span style={{ fontWeight: '900', color: '#ffffff', fontSize: '1.2rem' }}>{formatPrice(item.cost)}</span>
                <button style={{ padding: '10px 20px', background: '#3b82f6', color: '#ffffff', border: 'none', borderRadius: '12px', fontWeight: '800', fontSize: '0.85rem' }}>Plan ⚡</button>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <section style={{ padding: '80px 0', background: '#050505', position: 'relative', zIndex: 100 }}>
    ...
    <section className="featured-section">
      <div className="section-header-v2 container">
        <div className="sh-left">
          ...
          <h2 style={{ color: '#fff' }}>Top Rated Itineraries</h2>
        </div>
      ...
      <div className="featured-grid">
        {trips.map((trip) => (
          <div key={trip.id} className="featured-card" style={{ opacity: 1, visibility: 'visible' }}>
...
const Home = () => {
  ...
  const destinations = [
    ...
    { name: "Manali", tagline: "Himalayan Snow Peaks", img: "https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?q=80&w=800" },
    { name: "Kerala", tagline: "Backwaters & Tea", img: "https://images.unsplash.com/photo-1593179241557-bce1eb92e47e?q=80&w=800" },
    { name: "Jaipur", tagline: "Royal Pink City", img: "https://images.unsplash.com/photo-1599661046289-e31897846e41?q=80&w=800" },
  ];

  return (
    <div className="home-redesign">
      <div className="interactive-bg" style={{ zIndex: 0 }}>
        <motion.div 
          className="bg-blob"
      ...
      <section className="container onboarding-decision-section" style={{ position: 'relative', zIndex: 10 }}>
      ...
      <section className="container" style={{ position: 'relative', zIndex: 10, paddingBottom: '100px' }}>
        <div className="section-header">
          <h2>Trending Now</h2>
        </div>
